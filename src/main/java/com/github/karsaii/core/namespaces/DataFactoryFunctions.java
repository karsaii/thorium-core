...
public interface DataFactoryFunctions {
    ...
    static <T> Data<T> getWith(T object, boolean status, MethodMessageData messageData) {
        return getWith(object, status, messageData, DataFactoryConstants.DEFAULT_EXCEPTION);
    }
    ...
    static Data<Boolean> getBoolean(boolean status, String nameof, String message, Exception exception, String exceptionMessage) {
        return areNotNull(message, nameof, exception, exceptionMessage) && status ? getWith(status, status, nameof, message, exception, exceptionMessage) : CoreDataConstants.NULL_BOOLEAN;
    }
